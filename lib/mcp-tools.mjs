...
export function handleTool(name, args) {
  switch (name) {
    ...
    case 'read_range': {
      ...
      let returned_end_idx = paras.length > 0 ? paras[0].idx : start_idx;
      ...
      for (let i = 0; i < paras.length; i++) {
        ...
        if (parts.length === 0 && segment.length > max_chars) {
          ...
          const sliced = rawContent.slice(0, availableForContent);
      ...
      const returned_start_idx = paras.length > 0 ? paras[0].idx : start_idx;

      let comments = [];
      if (include_comments && parts.length > 0) {
        if (partial_paragraph) {
          // Partial paragraph: only return comments for that one paragraph
          comments = db.prepare('SELECT * FROM book_comments WHERE book_id = ? AND paragraph_idx = ? ORDER BY created_at').all(book_id, returned_start_idx);
        } else {
          const returnedIdxSet = new Set(paras.slice(0, parts.length).map(p => p.idx));
          comments = db.prepare('SELECT * FROM book_comments WHERE book_id = ? AND paragraph_idx >= ? AND paragraph_idx <= ? ORDER BY paragraph_idx, created_at').all(book_id, returned_start_idx, returned_end_idx)
